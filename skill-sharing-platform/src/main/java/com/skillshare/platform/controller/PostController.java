...
public class PostController {
    ...
    @GetMapping
    public ResponseEntity<?> getAllPosts(@AuthenticationPrincipal OAuth2User principal) {
        if (principal == null) {
    ...
    @PostMapping("/{id}/like")
    public ResponseEntity<Map<String, Object>> likePost(@PathVariable Long id, @AuthenticationPrincipal OAuth2User principal) {
        String email = principal.getAttribute("email");
        Map<String, Object> response = postService.likePost(id, email); // change return type in service
    ...
    public ResponseEntity<List<SharedPostDTO>> getMySharedPosts(
            @AuthenticationPrincipal OAuth2User principal) {
